...
5. **书签**：访问到一个页面后，点地址栏右侧 ☆ → 变实心 ★（已收藏）；新建一个标签，其起始页「书签」区
   ...
   - **跨标签书签入口**：收藏后开一个新标签页（不回起始页），点工具栏地址栏右侧的书签按钮（☰），
     应弹出悬浮列表列出所有书签，点其中一条能直接跳转、无需先回到起始页。
   - **地址栏自动补全**：在地址栏输入几个字符（能匹配某条书签的 URL 或标题），应弹出下拉建议列表
     （书签项带 ★ 标记）；方向键可上下选中，回车跳转到选中项；输入没有任何匹配时不弹出下拉，
     回车按原字符串走正常导航逻辑。
