...
final class FeatureVectorTests: XCTestCase {
    ...
    func data(_ hex: String) -> Data { Data(Hex.decode(hex)!) }

    // ---- 加密：四步对齐（地址 → x25519 中间值 → 共享密钥 → 密文 memo）----
    func testX25519PrivFromSeed() {
        let p = Encryption.x25519PrivFromSeed(Hex.decode(aSeedHex)!)
        XCTAssertEqual(Hex.encode(p), "70788f1a0cea001a2631dae5d05dbd062008d5b30f50b9e29beb2a7822289044")
    }

    func testEdToMontgomeryPub() {
        let bPub = Hex.decode(String(bAddr.dropFirst(2)))!
        let u = Encryption.montgomeryUFromEdPub(bPub)
    ...
    func testSharedKeySymmetric() {
        ...
        XCTAssertEqual(kAB.map(Hex.encode), sharedGold, "共享密钥与金标准不一致")
        XCTAssertEqual(kBA.map(Hex.encode), sharedGold, "ECDH 不对称")
    }

    func testHChaCha20Subkey() {
        let key = Hex.decode(sharedGold)!
        let subkey = Encryption.hchacha20(key: key, nonce16: Hex.decode("aa".replicated(16))!)
    ...
    func testFixedNonceCiphertextMatchesGolden() throws {
        ...
        let key = Hex.decode(sharedGold)!
        let nonce = Hex.decode("aa".replicated(24))!
        let sealed = try Encryption.aeadSeal(plaintext: Array("hi 🔐".utf8), key: key, xnonce: nonce)
        XCTAssertEqual(Hex.encode(sealed), "6359b5d168414e050a885e42c9dc6eabf98ecbaea44fa9")
        let memo = Config.encPrefix + Hex.encode(nonce + sealed)
        XCTAssertEqual(memo,
    ...
    func testNamesRegistry() throws {
        ...
        let claimAlice = try TxBuilder.transfer(wallet: x, to: x.address, amount: 1, nonce: 0, memo: "NAME|Alice", fee: 1)
        let yAlice = try TxBuilder.transfer(wallet: y, to: y.address, amount: 1, nonce: 0, memo: "NAME|alice", fee: 1)
        let yBob = try TxBuilder.transfer(wallet: y, to: y.address, amount: 1, nonce: 1, memo: "NAME|bob", fee: 1)
        ...
        XCTAssertFalse(Names.isValidName("treasury"))   // 保留名
    ...
    func testMarketParse() throws {
        let x = Wallet.generate(), y = Wallet.generate()
        let sell = try TxBuilder.transfer(wallet: x, to: x.address, amount: 1, nonce: 0, memo: "MKT|20|复习笔记", fee: 1)
        let buy = try TxBuilder.transfer(wallet: y, to: x.address, amount: 20, nonce: 0, memo: "BUY|\(sell.txid)", fee: 1)
    ...
    func testRedPacketEscrowAndBalances() throws {
        ...
        let red = try TxBuilder.transfer(wallet: x, to: Config.redEscrowAddress, amount: 100, nonce: 0, memo: "RED|2|r", fee: 1)
        let claimY = try TxBuilder.transfer(wallet: y, to: y.address, amount: 0, nonce: 0, memo: "CLAIM|\(red.txid)", fee: 1)
        let claimZ = try TxBuilder.transfer(wallet: z, to: z.address, amount: 0, nonce: 0, memo: "CLAIM|\(red.txid)", fee: 1)
        ...
        XCTAssertEqual(st.balance(Config.redEscrowAddress), 0, "全部抢完后托管必须清零")
        XCTAssertEqual(st.balance(x.address), 1000 - 100 - 1, "发起人付 总额+手续费")
        XCTAssertEqual(st.balance(y.address) + 1 + st.balance(z.address) + 1, 100, "两份之和 = 总额（各扣 1 手续费）")
        ...
        XCTAssertEqual(st.balance(y.address), claimedY - 1)
    }

    func testRedPacketRefund() throws {
        let x = Wallet.generate()
        let mint = coinbase(to: x.address, amount: 1000, index: 0)
        let red = try TxBuilder.transfer(wallet: x, to: Config.redEscrowAddress, amount: 100, nonce: 0, memo: "RED|5|e", fee: 1)
        let refund = try TxBuilder.transfer(wallet: x, to: x.address, amount: 0, nonce: 1, memo: "REFUND|\(red.txid)", fee: 1)
        let st = Chain.computeState([block([mint], hash: "00aa"), block([red], hash: "00bb"), block([refund], hash: "00cc")])
        XCTAssertEqual(st.balance(Config.redEscrowAddress), 0)
        XCTAssertEqual(st.balance(x.address), 1000 - 1 - 1, "退款取回全部，仅净付两笔手续费")
    ...
    func coinbase(to: String, amount: Int, index: Int) -> Transaction {
        let id = TxBuilder.txid(from: Config.nullAddress, to: to, amount: amount, fee: 0,
                                nonce: index, timestamp: 0, memo: "", burn: nil)
...
private extension String {
    /// "aa".replicated(24) → 24 个 "aa" 拼接（hex 串）。
