...
final class GoldVectorTests: XCTestCase {
    ...
    func testEncryptedMessage512MemoSelfValid() throws {
        let a = try wallet()
        let bAddr = "0x" + String(repeating: "ab", count: 32)
        // 长明文 → ENC| 密文 memo 远超 128 码点（memo 码点数 ≈ 84 + 2×明文字节）。
        let plaintext = String(repeating: "这是一条加密私信。", count: 3)   // 81 字节 → memo ≈ 246 码点
        let memo = try XCTUnwrap(Encryption.encryptMemo(plaintext, recipientAddress: bAddr, senderSeed: a.privateKey))
        ...
        let tx = try TxBuilder.message(wallet: a, to: bAddr, text: memo, nonce: 0,
                                       burn: Config.messageBurn, fee: 1, timestamp: ts)
    ...
    func testMemoLengthBoundaryAt512() throws {
        ...
        let ok = try TxBuilder.message(wallet: a, to: bAddr, text: memo512, nonce: 0, burn: 5, fee: 1, timestamp: ts)
        XCTAssertTrue(ok.selfValid(), "512 码点 memo 必须通过")
        let tooLong = try TxBuilder.message(wallet: a, to: bAddr, text: memo512 + "a", nonce: 0, burn: 5, fee: 1, timestamp: ts)
