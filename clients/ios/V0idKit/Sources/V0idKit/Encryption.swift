...
// 自检向量见 CLIENT-PROTOCOL §8.6 / §9（与 macOS 客户端同一组金标准对齐）。
//
// 依赖拆解（零新依赖）：
...
public enum Encryption {
    ...
    public static func decryptMemo(_ memo: String, otherPartyAddress: String, mySeed: Data) -> String? {
        ...
        guard let blob = Hex.decode(String(memo.dropFirst(Config.encPrefix.count))).map(Array.init), blob.count >= 24 + 16 else { return nil }
    ...
    static func sharedKey(mySeed: Data, otherAddress: String) -> [UInt8]? {
        ...
        guard let otherPub = Hex.decode(pubHex).map(Array.init), otherPub.count == 32 else { return nil }
