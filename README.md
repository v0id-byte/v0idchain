...
> 📘 **第一次玩？** 先看 **[怎么开节点 → docs/RUNNING-A-NODE.md](docs/RUNNING-A-NODE.md)**（含 Node.js 安装 / 版本要求 / 开节点全流程），再看 **[完整教程 → TUTORIAL.md](TUTORIAL.md)**（转账 / 集市 / 加密私信 / 红包 / 钱包备份，每条命令逐一讲解）。本 README 偏速查表与设计说明。
...
> **完整安装步骤（含 Node.js 升级）见 [docs/RUNNING-A-NODE.md](docs/RUNNING-A-NODE.md)。** 以下是速查版。
...
> 首次 `link` 若报 `Unable to find the global bin directory`，先跑一次 `corepack pnpm setup`（把 pnpm 全局 bin 目录写进 PATH，新开终端生效）再 link。改了 CLI 源码后重跑 `corepack pnpm build:cli` 刷新即可。不想装全局就在仓库根目录定义同名函数：`v0id() { corepack pnpm exec tsx packages/cli/src/index.ts "$@"; }`（带 ~1s 冷启动）。
...
> **双击** `mine.command`（macOS）或 `mine.bat`（Windows）可自动装依赖 + 开仪表盘 + 开挖，无需任何配置。

### 本地沙盒（双节点，不联公网）

```bash
# 终端 1
corepack pnpm dev:node1   # node1，挖矿

# 终端 2
corepack pnpm dev:node2   # node2，连上 node1
```

详细操作（查地址 / 转账 / 余额对比）见 [docs/RUNNING-A-NODE.md §6](docs/RUNNING-A-NODE.md#6-本地沙盒两个节点互联不联公网)。
...
```bash
corepack pnpm dev:web     # 浏览器 http://localhost:5173
```
...
```bash
corepack pnpm smoke          # 核心逻辑：挖矿/转账/余额/防重放/篡改检测/共识
corepack pnpm exec tsx scripts/integration.ts   # 多节点：广播/同步/追链/持久化
```

> 🧪 **想亲手当一次攻击者、看防御如何把你拒掉？** 见 **[攻防实验手册 → docs/LABS.md](docs/LABS.md)**——把篡改金额 / 压难度长 fork / 未来时间戳 / 双花 / 改坏链文件 / 越过 checkpoint 组织成 6 个可复现的动手实验（每个一条命令、附实测拒绝信息），可直接进分布式系统 / 密码学课当 lab。
...
v0id msg     <to> <文字…> [--burn <n>] [--fee <n>] [-e] [--api]   给一个地址发链上消息（不转币，烧 $V0ID 进虚空；--burn 默认 5；-e 端到端加密）
v0id inbox   [address] [--sent] [--api]       看收件箱（发给你的消息；--sent 改看发件箱）
v0id newcomers [--api]                        本次会话发现的新成员（新节点上线 / 新地址首次上链）

v0id name claim <name> [--api]                抢注一个全网唯一昵称（先到先得；自转 1 币 + memo）
v0id name list  [--api]                        看已注册的昵称
v0id name who   <name> [--api]                查某昵称属于哪个地址
v0id name of    [address] [--api]             查某地址的显示昵称
...
```bash
v0id market sell 30 复习笔记第3章 --api http://127.0.0.1:7001   # 上架（等一个区块确认）
v0id market list --api http://127.0.0.1:7002                    # 别的节点也能看到（已同步）
v0id market buy 9f59c01a --api http://127.0.0.1:7002            # 用 id 前缀购买
```
...
**发消息 = 烧币留言。** 给任意地址发一条消息：不转币，而是把一点 `$V0ID` **销毁进虚空地址**
（`NULL_ADDRESS`，永久不可花 = 销毁），消息正文明文上链、随链全网同步、永久可查。技术上它是一种新交易：
`amount=0 + burn>0 + memo=正文`，另付最低手续费给打包矿工。

```bash
v0id msg 0x720c…b1ce 在链上给你留个话 👋 --api http://127.0.0.1:7001   # 默认烧 5 + 1 gas
v0id inbox --api http://127.0.0.1:7002        # 收件人节点查收件箱（等一个区块确认）
v0id inbox --sent --api http://127.0.0.1:7001 # 看自己发出去的
```

全网累计销毁量 = 虚空地址余额，`v0id info` 的「已销毁 🔥」与仪表盘顶部都能看到（总账守恒：销毁额只是搬进永不可花的地址）。

**🔒 端到端加密私信。** 加 `-e` 让消息只有收件人能解（用对方公钥 x25519 ECDH + XChaCha20-Poly1305 加密，密文上链、旁人只看到 `ENC|` 乱码；发件人凭 ECDH 也能解自己发的）。仅**正文**加密，收发地址/时间/烧币额仍公开。

```bash
v0id msg 0x… "只有你能看到的悄悄话 🤫" -e --api http://127.0.0.1:7001   # 端到端加密
```

> 加密私信把 `MAX_MEMO` 从 128 抬到 512（容纳密文）——这是**软分叉**（旧节点拒收超 128 memo 的块），需全网一起升级；但不进哈希、不重置链。

**新人发现。** 运行中的节点会在两种「新人」出现时实时打一行 `🆕`，也可 `v0id newcomers` / 仪表盘「新成员」面板查看：

- **新节点上线**（P2P 层）：有新机器连进网络、握手报出地址时。
- **新地址首次上链**（经济身份）：某地址第一次作为转账/消息的收发方出现在区块里时。

**🪪 链上昵称（全网唯一抢注）。** 给地址起个名字，转账/消息/浏览器里从此显示 `@名字` 而不是一长串 `0x…`。抢注 = 自转 1 币 + memo `NAME|<名字>`，**先到先得**（同名只有第一个抢到者拥有）。名字 1~20 位 小写字母/数字/`_`/`-`，`treasury`/`official`/`admin` 等保留名禁注（防冒充）。

```bash
v0id name claim v0id-boss --api http://127.0.0.1:7001   # 抢注（等一个区块确认）
v0id name who  v0id-boss --api http://127.0.0.1:7002    # 别的节点也查得到 → 地址
v0id inbox --api http://127.0.0.1:7002                  # 收件箱里发件人直接显示 @v0id-boss
```

> 昵称是纯 memo 约定、**不改共识、不软分叉**：抢注交易在旧节点眼里只是合法自转，全网照收；只有想「显示」名字的客户端才需新代码。

> ⚠️ **消息是一次软分叉。** 不发消息时纯转账块新老节点都认；一旦有矿工把消息交易打进块，**没升级到本版本的节点会拒绝那个块**。所以联网玩消息前，请确保**所有节点（含公网种子）都升级**。好消息：创世 hash 与既有 checkpoint **完全不变**（销毁额字段仅在 >0 时计入 txid），老链与央行预挖原样有效，**无需重置链**。

---

## 和同学联机

最省事：所有人都 `corepack pnpm mine`，自动接入公共种子 `ws://mc.void1211.com:6001`，互相转账只需对方 `info` 里的地址。

局域网 / 自建种子 / 跨网的详细配置见 [docs/RUNNING-A-NODE.md §7](docs/RUNNING-A-NODE.md#7-开自己的公网种子节点) 与 [TUTORIAL.md §9](TUTORIAL.md#9-和同学联机-)。
...
- **本机 API 双重设防**：HTTP API 只绑 `127.0.0.1`，CORS 只放行本机（`localhost`/`127.0.0.1`）页面（防浏览器 CSRF）；**写接口（转账/挖矿/连接/集市）还需 Bearer 令牌**——`.data/<node>/api.token`（随机 32 字节，`0600`，启动自动生成；CLI 自动读取，仪表盘手动粘贴一次）。这挡住了**其他本地用户**直接 `POST /send` 盗币：他们读不到 `0600` 的 `api.token`，令牌也**不经任何接口下发**（曾有 `/my-token` 无鉴权回吐令牌，已删除）。⚠️ 但**以你自己身份运行的进程**（如恶意 npm postinstall）仍能直接读 `api.token`——`0600` 只挡别的用户、挡不住你自己的进程，这是文件令牌的固有边界。只读 GET 与 `/health` 不设防。
- **P2P 加固**：gossip 学来的 peer 地址会过滤掉私网/环回/链路本地地址（防被诱导去拨内网服务，SSRF 类；运营者 `--peers`/`--advertise` 显式地址走 trusted 通道、例外）。**IPv6 字面量按“只放行全局单播 `2000::/3`”白名单处理**，堵住 `::ffff:` IPv4-mapped、`64:ff9b::` NAT64、`[::1]` 等绕过写法（Node 会把 `::ffff:127.0.0.1` 规整成带方括号的十六进制，逐前缀黑名单不可靠）。`knownUrls` 满时 FIFO 淘汰最早的“非置顶”项、运营者种子永久置顶（防被垃圾地址挤爆而连不上真节点）；单条 WS 消息 ≤ 64MB（防巨型 JSON OOM）；**分片整链同步的聚合缓冲也有上限**（单连接 ≤ `MAX_SYNC_BLOCKS` 块，挡住已连 peer 把 `BLOCKS` 分片的 `total` 自报成天文数字、零成本撑爆内存的 pre-auth OOM）；`mempool` ≤ `MAX_MEMPOOL`（手续费已给 spam 定价，这里再加硬上限兜底反灌）。
- **本机数据文件 `0600`**：`wallet.json`（明文私钥）、`api.token`、以及 `chain.json` / `peers.json` 都收紧到仅属主可读写——后两者虽是公开数据（账本 / 公网 peer 地址），但与私钥、令牌统一权限、不给同机其他用户留可读副本；存量旧文件在加载时回填 `0600`。
- **`chain.json` 损坏兜底**：加载失败（解析错误或整链校验不过）时**先把坏文件改名备份**（`chain.json.corrupt-<时间戳>`）再从创世重建，绝不静默清空——否则有人改一个字节就能让节点重启即丢光本地状态。
- **仍然没有**：Sybil 抗性、TLS、节点间鉴权/加密、**成熟的**手续费市场（已有“高者优先 + 每块上限”的雏形，但还没有动态最低费 / EIP-1559 式定价）。两个已知低危残留：**HELLO 的 `address` 字段无签名**（节点身份可冒充，仅影响 peer 列表展示与去重，盗不了币——花钱仍需 ed25519 签名）；**WS 单条消息大小 + 分片同步聚合缓冲都有上限，但 inbound 连接数本身无上限、也无逐连接带宽限流**（出站被 `maxPeers=8` 约束）。这是教学链，**别上真钱**；真要联公网，至少自行套 TLS（如置于反向代理后）、管好 `api.token` 与央行私钥，并理解“低算力下 PoW 链固有的 51% 风险”。
...
- [x] Phase 10 — **链上消息**：给地址发消息 = 烧 `$V0ID` 进虚空（`amount=0 + burn>0 + memo`，销毁额向后兼容计入 txid → 创世/checkpoint 不变）· 收件箱/发件箱 · **新人发现**（新节点上线 + 新地址首次上链，CLI 实时 `🆕` + `newcomers` + 仪表盘面板）
