...
```bash
corepack pnpm v0id market sell 30 复习笔记第3章 --api http://127.0.0.1:7001   # 上架（等一个区块确认）
corepack pnpm v0id market list --api http://127.0.0.1:7002                    # 别的节点也能看到（已同步）
corepack pnpm v0id market buy 9f59c01a --api http://127.0.0.1:7002            # 用 id 前缀购买
```
...
```bash
corepack pnpm v0id msg 0x720c…b1ce 在链上给你留个话 👋 --api http://127.0.0.1:7001   # 默认烧 5 + 1 gas
corepack pnpm v0id inbox --api http://127.0.0.1:7002        # 收件人节点查收件箱（等一个区块确认）
corepack pnpm v0id inbox --sent --api http://127.0.0.1:7001 # 看自己发出去的
```
...
```bash
corepack pnpm v0id msg 0x… "只有你能看到的悄悄话 🤫" -e --api http://127.0.0.1:7001   # 端到端加密
```
...
```bash
corepack pnpm v0id name claim v0id-boss --api http://127.0.0.1:7001   # 抢注（等一个区块确认）
corepack pnpm v0id name who  v0id-boss --api http://127.0.0.1:7002    # 别的节点也查得到 → 地址
corepack pnpm v0id inbox --api http://127.0.0.1:7002                  # 收件箱里发件人直接显示 @v0id-boss
```
