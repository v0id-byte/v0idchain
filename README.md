...
```bash
$v msg 0x720c…b1ce 在链上给你留个话 👋 --api http://127.0.0.1:7001   # 默认烧 5 + 1 gas
$v inbox --api http://127.0.0.1:7002        # 收件人节点查收件箱（等一个区块确认）
$v inbox --sent --api http://127.0.0.1:7001 # 看自己发出去的
```
...
```bash
$v msg 0x… "只有你能看到的悄悄话 🤫" -e --api http://127.0.0.1:7001   # 端到端加密
```
...
```bash
$v name claim v0id-boss --api http://127.0.0.1:7001   # 抢注（等一个区块确认）
$v name who  v0id-boss --api http://127.0.0.1:7002    # 别的节点也查得到 → 地址
$v inbox --api http://127.0.0.1:7002                  # 收件箱里发件人直接显示 @v0id-boss
```
