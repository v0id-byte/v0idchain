...
- **最大工作量链共识**：收到**累计 PoW 工作量更大**的合法链才整体替换（`chainWork = Σ 2^difficulty`），**而非比谁更长**——否则攻击者只要压低难度、狂出低难度块凑长度就能反超诚实链造成双花。工作量相等或更小不替换（先到先得）。短暂分叉随后被工作量更大的链收敛。
...
- [x] Phase 8 — **安全加固**：最大工作量共识 + 未来时间戳上限（防压难度双花）· API 令牌鉴权 · P2P 私网过滤 / `knownUrls` 置顶 FIFO · WS 大小上限 · mempool 上限 · 私钥/令牌文件 `0600`
