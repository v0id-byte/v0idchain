...
// Mixnet 掩护 cell（Phase 2C）。线缆上它就是一个普通 512B 加密 RELAY cell——对任何观察者/中继与 CMD_DATA 不可区分
// （cmd 字节在洋葱密文内，唯有**终点跳**剥到本层后才看见 cmd===CMD_DROP）。终点见之**静默丢弃**：不投递给任何
// app/出口/hs/rdv，也不回任何东西。中间跳永远看不出它是 drop，照常盲转发。用途：让客户端电路恒有流量，
// 全局观察者无法据“何时有 cell”推断“何时有真数据”。⚠️ 绝不用 cells.ts 的 cv? 线缆字段标记 cover——那会让 cover
// 在明文线缆里可分辨（直接泄露哪些是掩护），破坏不可区分性；真掩护必须是“加密层内不可分辨”，即此 CMD_DROP 方案。
export const CMD_DROP = 20; // 客户端→终点：掩护(cover)cell，终点静默丢；中间跳当普通 cell 转发
