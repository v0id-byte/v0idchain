...
### §4.3 v1 取向：建好但先不发
国库预挖有限（~1000）。v1 把测量 + 奖励计算 + 罚没工具建好测好，**暂不实际发奖、不烧国库**——质押门控（必须质押才被选路）本身已是女巫成本。等激活 + 外部运营者来了再开发放；可持续闭环靠 §8 客户端付费。
...
1. **客户端概率支付（Orchid 纳米支付）**：用户按流量签彩票、中继兑中奖票上链 → 密码学抗女巫、无需中心测量方 = 真经济闭环 + 去中心化测量终局。
